 ...
 */
export const getConfigFromEnv = <S extends ZodV3ObjectLike>({ schema, debug = false, envSources, delimiter }: GetConfigFromEnvOptions<S>): InferZodV3Output<S> => {
  return getConfigFromEnvCore<InferZodV3Output<S>>({ debug, envSources, delimiter }, schema, zodV3Adapter);
};
